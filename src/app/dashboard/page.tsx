...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    // 🚀 Fetch the user from local storage to check probation
    const storedUser = localStorage.getItem("coop_user");
    if (storedUser) {
      const parsedUser = JSON.parse(storedUser);
      setUser(parsedUser);

      // 🚀 Calculate the exact 6-month probation end date
      const joinDate = new Date(parsedUser.dateJoined || parsedUser.createdAt);
      const endDate = new Date(joinDate);
      endDate.setMonth(endDate.getMonth() + 6);

      // 🚀 Determine if current date is still under the 6 months
      if (new Date() < endDate) {
